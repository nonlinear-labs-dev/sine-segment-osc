...
class SynthProxy:
    ...
    def updateOsc(self, v):
        ...
        sliders[9].set(sliders[16].get())
        sliders[10].set(sliders[16].get())

        sliders[15].set(sliders[16].get())
        sliders[14].set(sliders[16].get())
...
class TkUI(UI):
    ...
    def create_widgets(self):
        ...
        for i in range(0, 7):
            pos = tk.Scale(self, from_=0.05, to=0.95, resolution=0.0001, orient='horizontal', command=synth.updateOsc)
            self.osc_sliders.append(pos)

        # intesity slider
        for i in range(1):
            higherlevel = tk.Scale(self, from_=0.05, to=0.95, resolution=0.0001, orient='horizontal', command=synth.updateOsc)
            higherlevel.set(0.5)
            self.osc_sliders.append(higherlevel)
