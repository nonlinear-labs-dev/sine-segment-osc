...
class TkUI(UI):
    ...
    def create_widgets(self):
        ...
        self.morph_slider = []
        # 16 is hardcoded
        self.savedValuesOsc = np.zeros((4, 16))
        self.currentValues = []
        ...
        for i in range(0, 7):
            pos = tk.Scale(self, from_=0.0001, to=1, resolution=0.0001, orient='horizontal', command=synth.updateOsc)
        ...
        FourthSine = partial(self.makePreset, 2)
        self.sine = tk.Button(self, text="Fourth Sine", command=FourthSine)
        self.sine.place(x=1300, y=40, width=150, height=40)

        HalfSine = partial(self.makePreset, 3)
        self.sine = tk.Button(self, text="Half Sine", command=HalfSine)
        self.sine.place(x=1300, y=80, width=150, height=40)

        Ocean = partial(self.makePreset, 4)
        self.sine = tk.Button(self, text="Ocean", command=Ocean)
        ...
        if(a==3):
            for i in range(8):
                if(i % 2 == 0):
                    ...
                    self.osc_sliders[i].set(-1)
            self.osc_sliders[9].set(0.0001)
            self.osc_sliders[10].set(0.5)
            self.osc_sliders[14].set(0.5)
            self.osc_sliders[15].set(0.5)

            self.osc_sliders[11].set(0.0001)
            self.osc_sliders[12].set(0.0001)
            self.osc_sliders[13].set(0.0001)
      
        if(a==4):
            self.osc_sliders[0].set(0)
            self.osc_sliders[1].set(0)
            self.osc_sliders[2].set(0.52)
            self.osc_sliders[3].set(0)
            self.osc_sliders[4].set(1)
            self.osc_sliders[5].set(0.61)
            self.osc_sliders[6].set(0)
            self.osc_sliders[7].set(1)
            self.osc_sliders[9].set(0.5)
            self.osc_sliders[10].set(0.5)
            self.osc_sliders[14].set(0.5)
            self.osc_sliders[15].set(0.237)
            self.osc_sliders[11].set(0.491)
            self.osc_sliders[12].set(0.133)
            self.osc_sliders[13].set(0.410)            
...
class Presets:
    ...
    def WritePreset():
        ...
        save_dialog = filedialog.asksaveasfile(mode='w', defaultextension=".csv")
        np.savetxt(save_dialog.name, all_values, delimiter=',')
    ...
    def WriteCombination():
        ...
        save_dialog = filedialog.asksaveasfile(mode='w', defaultextension=".csv")
        np.savetxt(save_dialog.name, all_values_comb, delimiter=',')
